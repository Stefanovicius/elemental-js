...
const handleAttribute = (element, attribute, valueArr) => {
  ...
  valueArr.forEach((value) => {
    if (isReactive(value)) value.subscribe(updateAttribute)
  })
}

const handleAttributes = (element, attributes) => {
  Object.entries(attributes).forEach(([attribute, value]) => {
    if (attribute.startsWith('on')) handleEventListeners(element, attribute.slice(2), value)
...
const elementsEqual = (a, b) => {
  ...
  if (a instanceof HTMLElement && b instanceof HTMLElement) return a.outerHTML === b.outerHTML
  return false
}
...
const handleContent = (element, contentArr) => {
  ...
  contentArr.forEach((content) => {
    if (isReactive(content)) content.subscribe(() => updateContent(element, contentArr))
  })
}
