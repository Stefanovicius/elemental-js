...
const processContent = (content) => {
  ...
  if (content instanceof DocumentFragment)
    return Array.from(content.childNodes).flatMap(processContent)
...
const updateContent = (element, contentArr) => {
  ...
  newContent.forEach((newChild, index) => {
    ...
    if (!existingChild) element.appendChild(newChild)
    else if (!elementsEqual(newChild, existingChild)) element.replaceChild(newChild, existingChild)
  })
...
export const createElement = ({ tag, attributes, content }) => {
  ...
  tag && handleAttributes(element, attributes)
